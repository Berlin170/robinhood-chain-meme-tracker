...
// ── the genesis mint (Transfer from the zero address) inside a launch
// tx always names the real deployed token contract first — the one
// thing on a launch row a user can actually verify elsewhere
// (CoinMarketCap, another explorer) instead of trusting the name/symbol
// a copycat could just as easily claim. cached long since a mined tx's
// transfers never change. ──
export function getTokenTransfers(txHash) {
  return bs(`/transactions/${txHash}/token-transfers`, 3600);
}
