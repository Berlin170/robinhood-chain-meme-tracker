...
import { getTokenInfo, getFactoryLaunches, getTokenTransfers } from "./blockscout";
...
// txHash -> deployed token address. module-scoped so it survives across
// requests in the same server process — a mined tx's transfers never
// change, so once a launch's address is resolved there's no reason to
// pay for that lookup again on the next 15s revalidate.
const tokenAddressCache = new Map();

async function resolveTokenAddress(txHash) {
  if (tokenAddressCache.has(txHash)) return tokenAddressCache.get(txHash);
  try {
    const transfers = await getTokenTransfers(txHash);
    const address = transfers?.items?.[0]?.token?.address_hash;
    tokenAddressCache.set(txHash, address);
    return address;
  } catch (err) {
    return undefined;
  }
...
async function buildNewLaunches() {
  ...
  // resolve real contract addresses only for what's actually displayed —
  // resolving all last-hour launches would mean 60-80+ extra Blockscout
  // calls every 15s instead of a bounded 30.
  const top30 = tagLaunchRisks(combined).slice(0, 30);

  return Promise.all(
    top30.map(async (l) => {
      const address = await resolveTokenAddress(l.txHash);
      ...
      return {
        ...
        flags: l.flags,
        address,
        addressShort: address ? `${address.slice(0, 6)}…${address.slice(-4)}` : undefined,
        blockscoutUrl: address ? `https://robinhoodchain.blockscout.com/token/${address}` : undefined,
      };
    })
  );
}
